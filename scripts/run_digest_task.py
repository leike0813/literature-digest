...
# Add the script directory to sys.path to import provenance
current_dir = Path(__file__).parent
sys.path.append(str(current_dir))

from provenance import utc_now_iso, sha256_file
...
def main():
    # Simulated Payload
    md_path_str = "examples/example_full.md"
    parent_key = "EXAMPLE_KEY"
    attach_key = "EXAMPLE_ATTACH"
    ...
    if not md_path.exists():
        print(json.dumps({
            ...
            "parent_itemKey": parent_key,
            "md_attachment_key": attach_key,
            "digest_path": "",
            "references_path": "",
            "provenance": {"generated_at": utc_now_iso(), "input_hash": ""},
            "warnings": [],
            "error": {"code": "FILE_NOT_FOUND", "message": f"File not found: {md_path_str}"}
        }, ensure_ascii=False))
        return
    ...
    # 2. Digest (Hardcoded for this example as per process_paper.py)
    digest_content = """## TL;DR
...
5. **结论**：总结方法优劣及未来改进方向。"""
    ...
    # Use absolute paths for output in JSON to be safe/explicit, or relative. 
    # AGENTS.md doesn't specify, but absolute is usually safer for automated tools.
    # However, for this CLI interaction, relative to CWD is fine.
    
    digest_filename = f"{md_path.stem}_digest.md"
    refs_filename = f"{md_path.stem}_references.json"
    
    digest_path = output_dir / digest_filename
    refs_path = output_dir / refs_filename
    
    digest_path.write_text(digest_content, encoding='utf-8')
    refs_path.write_text(json.dumps(references, ensure_ascii=False, indent=2), encoding='utf-8')
    ...
    response = {
        ...
        "provenance": {
            ...
            "input_hash": sha256_file(md_path)
        },
        "warnings": [],
        "error": None
    }
